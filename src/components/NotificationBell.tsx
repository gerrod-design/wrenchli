...
export default function NotificationBell() {
  const { alerts, unreadCount, markAsRead, markAllAsRead } = useRecallAlerts();
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        aria-label={`${unreadCount} unread recall alerts`}
      >
        <Bell className="h-5 w-5 text-primary-foreground/80" />
        <span className="absolute -right-0.5 -top-0.5 flex h-4.5 w-4.5 items-center justify-center rounded-full bg-destructive text-[10px] font-bold text-destructive-foreground min-w-[18px] h-[18px] px-1">
      ...
      {open && (
        <div className="absolute right-0 top-full z-50 mt-2 w-80 max-h-96 overflow-y-auto rounded-lg border border-border bg-card shadow-xl">
          <div className="flex items-center justify-between border-b border-border px-4 py-3">
            <h3 className="text-sm font-semibold text-card-foreground">Recall Alerts</h3>
            {unreadCount > 1 && (
              <Button
                variant="ghost"
                size="sm"
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="mt-1 text-[10px] text-muted-foreground/60">
                      Campaign #{alert.campaign_number}
                    </p>
                  </div>
                  <button
