...
// Shared across InsightsTab, chatbot, and other vehicle intelligence features
...
export const RELIABLE_BRANDS = ["Honda", "Toyota", "Mazda", "Subaru", "Lexus"];

/**
 * Get known issues relevant to a vehicle at a specific mileage.
 * Returns issues where mileage is within range (with 15k buffer before minMiles).
 */
export function getRelevantIssues(make: string, mileage?: number | null) {
  const issues = VEHICLE_KNOWN_ISSUES[make] || [];
  if (!mileage) return { relevant: issues, upcoming: [] };

  const relevant = issues.filter(
    (i) => mileage >= i.minMiles - 15000 && mileage <= i.maxMiles
  );
  const upcoming = issues.filter(
    (i) => mileage < i.minMiles - 15000
  );

  return { relevant, upcoming };
}
