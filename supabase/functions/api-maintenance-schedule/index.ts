...
interface MaintenanceItem {
  type: string; label: string; interval_miles: number; interval_months: number;
  estimated_cost_low: number; estimated_cost_high: number;
  priority: "essential" | "recommended" | "optional"; description: string;
}
...
const BRAND_INTERVALS: Record<string, Record<string, Partial<MaintenanceItem>>> = {
  BMW: { oil_change: { interval_miles: 10000, estimated_cost_low: 80, estimated_cost_high: 150 }, brake_inspection: { estimated_cost_low: 50, estimated_cost_high: 120 }, spark_plugs: { estimated_cost_low: 250, estimated_cost_high: 500 } },
  "Mercedes-Benz": { oil_change: { interval_miles: 10000, estimated_cost_low: 90, estimated_cost_high: 160 }, spark_plugs: { estimated_cost_low: 280, estimated_cost_high: 550 } },
  Audi: { oil_change: { interval_miles: 10000, estimated_cost_low: 80, estimated_cost_high: 145 }, spark_plugs: { estimated_cost_low: 250, estimated_cost_high: 480 } },
  Tesla: { oil_change: { interval_miles: 0, label: "N/A (Electric)", estimated_cost_low: 0, estimated_cost_high: 0 }, spark_plugs: { interval_miles: 0, label: "N/A (Electric)", estimated_cost_low: 0, estimated_cost_high: 0 }, serpentine_belt: { interval_miles: 0, label: "N/A (Electric)", estimated_cost_low: 0, estimated_cost_high: 0 }, transmission_service: { interval_miles: 0, label: "N/A (Electric)", estimated_cost_low: 0, estimated_cost_high: 0 } },
};
...
function getScheduleForVehicle(make: string): MaintenanceItem[] {
  ...
  return MAINTENANCE_SCHEDULE.map((item) => {
    ...
    if (override) { const merged = { ...item, ...override }; if (merged.interval_miles === 0) return null; return merged; }
    return item;
  }).filter((item): item is MaintenanceItem => item !== null);
}

interface UpcomingItem {
  type: string; label: string; due_mileage: number; miles_until_due: number;
  priority: "overdue" | "urgent" | "soon" | "upcoming";
  estimated_cost_low: number; estimated_cost_high: number; description: string;
}

function getUpcoming(schedule: MaintenanceItem[], currentMileage: number, lastServices: { type: string; mileage: number }[]): UpcomingItem[] {
  return schedule.map((item) => {
    ...
    return { type: item.type, label: item.label, due_mileage: dueMileage, miles_until_due: milesUntilDue, priority, estimated_cost_low: item.estimated_cost_low, estimated_cost_high: item.estimated_cost_high, description: item.description };
...
serve(async (req) => {
  ...
  if (!apiKey) {
    return new Response(JSON.stringify({ error: "Missing API key. Include x-api-key header." }), { status: 401, headers: { ...securityHeaders, "Content-Type": "application/json" } });
  }
  ...
  try {
    ...
    if (!vehicle || !vehicle.make || !vehicle.mileage) {
      return new Response(JSON.stringify({ error: "Vehicle make and mileage are required." }), { status: 400, headers: { ...securityHeaders, "Content-Type": "application/json" } });
    }
    if (vehicle.mileage < 0 || vehicle.mileage > 500000) {
      return new Response(JSON.stringify({ error: "Invalid mileage (0–500,000)." }), { status: 400, headers: { ...securityHeaders, "Content-Type": "application/json" } });
    }
    ...
    const response = {
      ...
      summary: { total_items: upcoming.length, overdue_count: overdueCount, urgent_count: urgentCount, estimated_total_cost_low: totalCostLow, estimated_total_cost_high: totalCostHigh },
      wrenchli_services: { garage_url: `${baseUrl}/garage`, vehicle_insights_url: `${baseUrl}/vehicle-insights` },
    };

    supabase.from("api_request_logs").insert({
      endpoint: "api-maintenance-schedule", key_hash: keyHash, diagnosis_title: `${overdueCount} overdue, ${urgentCount} urgent`,
      vehicle_year: vehicle.year ? String(vehicle.year) : null, vehicle_make: vehicle.make, vehicle_model: vehicle.model ?? null,
      response_status: 200, response_time_ms: Date.now() - requestStart, cost_low: totalCostLow, cost_high: totalCostHigh,
    }).then(() => {});
