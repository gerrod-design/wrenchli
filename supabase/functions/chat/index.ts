...
// NOTE: Claude-facing names use assessment_* vocabulary to satisfy COPY CHECK.
// At the function-to-DB write boundary in executeTool(), assessment_title is
// mapped to the existing DB column diagnosis_title. DB schema unchanged
// (rename deferred to a separate workstream — see TASKS.md "This Month").
const tools = [
  {
    name: "assess_symptoms",
    description:
      "Run a symptom assessment for a vehicle using OBD2 codes or a plain-language symptom description. Returns likely causes, urgency, cost range, and DIY feasibility.",
    input_schema: {
      type: "object",
      properties: {
        codes: { type: "string", description: "OBD2 trouble codes, e.g. 'P0420 P0171'" },
  ...
  },
  {
    ...
    input_schema: {
      type: "object",
      properties: {
        assessment_title: { type: "string", description: "The repair / likely-cause title, e.g. 'Worn Brake Pads' or 'Faulty Catalytic Converter'. REQUIRED." },
        assessment_code: { type: "string", description: "DTC code if applicable, e.g. 'P0420'" },
      ...
      required: ["assessment_title", "zip_code"],
    },
  },
  {
  ...
  },
  {
    ...
    description:
      "Run a visual symptom assessment from photos of vehicle damage. Returns damage type, severity, affected area, repair options with cost estimates, and safety notes. Use when the user sends photos.",
    input_schema: {
      type: "object",
      properties: {
        image_urls: {
...
// ── System Prompt (v2 — COPY CHECK compliant) ──
// Source: /mnt/documents/chat-mike-prompt-v2.md (staged 2026-05-02, scan PASS)
...
- Wrenchli does symptom assessment, not diagnosis. NEVER use the words "diagnose," "diagnosis," "diagnoses," "diagnosing," or "diagnosed" in your responses.
- Use these words instead: "symptom assessment," "likely causes," "assessment results," "what's likely going on," "what we're seeing."
...
**TRIAGE LOGIC — CRITICAL (apply AFTER getting assessment results):**
...
- Tell them about the audio recording feature: "You can tap the 🎤 microphone button next to the chat input to record a short clip of the noise — I'll listen and help figure out what's likely going on!"
- You can also suggest uploading a VIDEO of the issue — videos automatically extract both visual frames AND audio, giving the best combined assessment.
- Audio clips are analyzed by AI and the results appear right in the chat.
- If a user sends a message starting with "🔊 [Recorded a car noise clip for analysis]", that means they just used the audio recorder. The next message will contain the AI's audio analysis — incorporate it into your assessment naturally.
- If a user sends a message starting with "🎬🔊 [Analyzed video:", that means they uploaded a video and both visual frames AND audio were analyzed together. The next message contains the combined analysis — use it as your primary input.
...
IMPORTANT: When calling estimate_repair_cost, use exact parameter names: "assessment_title", "vehicle_year", "vehicle_make", "vehicle_model", "zip_code".

**After assessment tool results — apply triage, then:**
...
// names are mapped to the existing DB column names diagnosis_*
// before downstream edge functions / DB writes.
  ...
  try {
    ...
    switch (name) {
      ...
      case "estimate_repair_cost": {
        ...
        if (args.assessment_title && !args.diagnosis_title) {
          args.diagnosis_title = args.assessment_title;
          delete args.assessment_title;
        }
        if (args.assessment_code && !args.diagnosis_code) {
          args.diagnosis_code = args.assessment_code;
          delete args.assessment_code;
        }
      ...
      case "search_repair_videos": {
        ...
        const repairTitle = String(rawArgs.repair_title || rawArgs.assessment_title || rawArgs.diagnosis_title || "vehicle repair");
