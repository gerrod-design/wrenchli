...
interface VehicleListing {
  id: string;
  spec: {
    make: string; model: string; year: number; trim?: string; bodyStyle?: string;
    engine?: string; transmission?: string; drivetrain?: string; fuelType?: string;
    mpgCity?: number; mpgHighway?: number;
  };
  price: number;
  mileage: number;
...
function getCacheKey(params: SearchParams): string {
  return JSON.stringify({
    z: params.zipCode, d: params.maxDistance ?? 50, y0: params.minYear,
    y1: params.maxYear, p: params.maxPrice, m: params.maxMileage, mk: params.makes?.sort(),
  });
}

async function searchMarketCheck(apiKey: string, params: SearchParams): Promise<VehicleListing[]> {
  ...
  if (params.makes?.length) url.searchParams.set("make", params.makes.join(","));
  ...
  if (!resp.ok) { console.error("MarketCheck error:", JSON.stringify(data)); return []; }
  ...
  return results.map((r: any) => {
    ...
    const apr = 5.9; const termMonths = 60;
    ...
    return {
      ...
      spec: {
        make: r.build?.make ?? r.make ?? "", model: r.build?.model ?? r.model ?? "",
        year: r.build?.year ?? r.year ?? 0, trim: r.build?.trim ?? r.trim,
        bodyStyle: r.build?.body_type ?? r.body_type, engine: r.build?.engine ?? r.engine,
        transmission: r.build?.transmission ?? r.transmission, drivetrain: r.build?.drivetrain ?? r.drivetrain,
        fuelType: r.build?.fuel_type ?? r.fuel_type, mpgCity: r.build?.city_miles, mpgHighway: r.build?.highway_miles,
      },
      price, mileage: r.miles ?? 0,
      location: { city: r.dealer?.city ?? "", state: r.dealer?.state ?? "", zipCode: r.dealer?.zip ?? params.zipCode, distance: r.dist },
      dealer: { name: r.dealer?.name ?? "Dealer", rating: r.dealer?.rating, reviewCount: r.dealer?.review_count },
      features: r.extra?.features ?? [], images: r.media?.photo_links?.slice(0, 5) ?? [], badges,
      monthlyPayment: price > 0 ? { amount: monthlyPayment, apr, termMonths, downPayment } : undefined,
      source: "marketcheck", sourceUrl: r.vdp_url ?? "", daysOnMarket: r.dom,
    } as VehicleListing;
  });
}
...
function getFallbackListings(params: SearchParams): VehicleListing[] {
  const listings: VehicleListing[] = [
    {
      id: "fb-1", spec: { make: "Honda", model: "Accord", year: 2019, trim: "LX", bodyStyle: "Sedan", engine: "1.5L Turbo I4", transmission: "CVT", drivetrain: "FWD", fuelType: "Gasoline", mpgCity: 30, mpgHighway: 38 },
      price: 22995, mileage: 45000, location: { city: "Detroit", state: "MI", zipCode: params.zipCode },
      dealer: { name: "Metro Honda", rating: 4.2, reviewCount: 847 },
      features: ["Backup Camera", "Bluetooth", "Honda Sensing", "Keyless Entry"], images: [], badges: ["certified", "one-owner"],
      monthlyPayment: { amount: 389, apr: 4.9, termMonths: 60, downPayment: 2000 },
      source: "fallback", sourceUrl: "https://www.autotrader.com/cars-for-sale/certified/honda/accord", daysOnMarket: 12,
    },
    {
      id: "fb-2", spec: { make: "Toyota", model: "Camry", year: 2020, trim: "LE", bodyStyle: "Sedan", engine: "2.5L I4", transmission: "Automatic", drivetrain: "FWD", fuelType: "Gasoline", mpgCity: 28, mpgHighway: 39 },
      price: 24450, mileage: 32000, location: { city: "Dearborn", state: "MI", zipCode: params.zipCode },
      dealer: { name: "Toyota of Dearborn", rating: 4.5, reviewCount: 1203 },
      features: ["Apple CarPlay", "Lane Assist", "Auto Emergency Braking"], images: [], badges: ["low-mileage", "warranty"],
      monthlyPayment: { amount: 425, apr: 3.9, termMonths: 60, downPayment: 2500 },
      source: "fallback", sourceUrl: "https://www.cars.com/vehicledetail/detail/456789", daysOnMarket: 8,
    },
    {
      id: "fb-3", spec: { make: "Ford", model: "Escape", year: 2018, trim: "SE", bodyStyle: "SUV", engine: "1.5L Turbo I4", transmission: "Automatic", drivetrain: "AWD", fuelType: "Gasoline", mpgCity: 23, mpgHighway: 30 },
      price: 19999, mileage: 52000, location: { city: "Southfield", state: "MI", zipCode: params.zipCode },
      dealer: { name: "Bill Brown Ford", rating: 3.9, reviewCount: 543 },
      features: ["AWD", "Heated Seats", "Sync 3", "Roof Rails"], images: [], badges: ["accident-free"],
      monthlyPayment: { amount: 349, apr: 5.9, termMonths: 60, downPayment: 1500 },
      source: "fallback", sourceUrl: "https://www.cargurus.com/Cars/inventorylisting/viewDetailsFilterViewInventoryListing.action", daysOnMarket: 23,
    },
  ];
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (cached && Date.now() - cached.ts < CACHE_TTL_MS) {
      ...
      if (marketCheckKey) {
        try { listings = await searchMarketCheck(marketCheckKey, params); }
        catch (err) { console.error("MarketCheck search failed, using fallback:", err); }
      } else {
    ...
    return new Response(
      JSON.stringify({ success: true, listings, total: listings.length, source: listings[0]?.source === "fallback" ? "fallback" : "marketcheck", cached: fromCache }),
