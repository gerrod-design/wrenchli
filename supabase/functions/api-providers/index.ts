...
serve(async (req) => {
  ...
  if (req.method !== "GET") {
    return new Response(JSON.stringify({ error: "Method not allowed. Use GET." }), { status: 405, headers: { ...securityHeaders, "Content-Type": "application/json" } });
  }
  ...
  try {
    ...
    if (!location) return new Response(JSON.stringify({ error: "Location parameter is required" }), { status: 400, headers: { ...securityHeaders, "Content-Type": "application/json" } });

    const providers = findServiceProviders({ location, service_type, radius_miles, price_range, vehicle_make });

    const response = {
      providers,
      search_params: { location, service_type, radius_miles, price_range, vehicle_make, results_count: providers.length },
      wrenchli_services: { compare_quotes_url: `${BASE_URL}/vehicle-insights?location=${encodeURIComponent(location)}&service=${service_type}`, book_service_url: `${BASE_URL}/for-shops`, get_more_options_url: `${BASE_URL}/vehicle-insights?location=${encodeURIComponent(location)}` },
    };

    supabase.from("api_request_logs").insert({ endpoint: "api-providers", key_hash: keyHash, diagnosis_title: service_type, zip_code: location, vehicle_make: vehicle_make || null, response_status: 200, response_time_ms: Date.now() - requestStart }).then(() => {});
